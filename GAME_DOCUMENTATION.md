...
7. [Chế độ Tracking](#7-chế-độ-tracking)
8. [Hệ thống Combo](#8-hệ-thống-combo)
9. [Hệ thống cài đặt (Settings)](#9-hệ-thống-cài-đặt-settings)
10. [Hệ thống âm thanh](#10-hệ-thống-âm-thanh)
11. [Hệ thống đồ họa & hiệu ứng](#11-hệ-thống-đồ-họa--hiệu-ứng)
...
**Mục tiêu**: Người chơi có thể chọn giữa 2 chế độ:
- **Classic**: Click chuột vào các mục tiêu xuất hiện ngẫu nhiên trước khi chúng biến mất. Liên tiếp bắn trúng để tăng combo multiplier.
- **Tracking**: Giữ crosshair trên mục tiêu di chuyển theo đường cong Bézier. Tốc độ tăng dần qua 3 giai đoạn.

**Vòng lặp gameplay chính (Classic)**:
1. Chọn chế độ → Chọn độ khó → Đếm ngược 3-2-1 → Bắt đầu chơi
2. Target xuất hiện 1 cái một lúc trên màn hình
3. Click trúng = Hit (được điểm + tăng combo), click trượt hoặc target hết hạn = Miss (reset combo)
4. Game kết thúc sau thời gian quy định (mặc định 60 giây)
5. Hiển thị kết quả chi tiết

**Vòng lặp gameplay chính (Tracking)**:
1. Chọn chế độ → Đếm ngược 3-2-1 (không chọn độ khó) → Bắt đầu chơi
2. Target di chuyển theo đường cong Bezier
3. Giữ crosshair trong vùng target để tích điểm
4. Tốc độ tăng qua 3 phase: Slow (0.2x) → Medium (0.5x) → Fast (1.0x)
5. Kết quả hiển thị accuracy và thời gian on-target
...
├── records.json              # Lưu kỷ lục accuracy theo từng độ khó/chế độ
...
│   ├── mode_select.py        # Chọn chế độ (Classic/Tracking)
│   ├── instruction.py        # Hướng dẫn chơi
│   ├── settings_screen.py    # Cài đặt (2 tab: General & Crosshair)
│   ├── difficulty.py         # Chọn độ khó (chỉ Classic)
│   ├── countdown.py          # Đếm ngược 3-2-1-GO!
│   ├── playing.py            # Màn hình chơi Classic (với combo)
│   ├── tracking.py           # Màn hình chơi Tracking
│   ├── game_over.py          # Hiển thị kết quả (theo chế độ)
...
### 3.4. Chọn chế độ (STATE_MODE_SELECT = 8)
- **File**: `screens/mode_select.py`
- **2 nút chọn chế độ**:
  - `CLASSIC` → Chuyển sang màn chọn độ khó
  - `TRACKING` → Chuyển thẳng sang đếm ngược (bỏ qua chọn độ khó)
- Mô tả ngắn gọn dưới mỗi nút
- **Nút**: `BACK` → Quay lại Menu

### 3.5. Chọn độ khó (STATE_DIFFICULTY = 3) — Chỉ Classic
- **File**: `screens/difficulty.py`
- **3 nút chọn**:
  - `EASY` (màu xanh lá)
  - `MEDIUM` (màu vàng cam)
  - `HARD` (màu đỏ)
- **Nút**: `BACK` → Quay lại chọn chế độ
- Sau khi chọn → chuyển sang đếm ngược

### 3.6. Đếm ngược (STATE_COUNTDOWN = 4)
...
- **Classic**: Hiển thị tên độ khó đã chọn
- **Tracking**: Không hiển thị độ khó
- Nhấn ESC để hủy (Classic → về chọn độ khó, Tracking → về chọn chế độ)
- Con trỏ chuột bị ẩn, chuột bị khóa trong cửa sổ

### 3.7. Màn hình chơi Classic (STATE_PLAYING = 5)
- **File**: `screens/playing.py`
- **Đây là màn hình gameplay Classic**, bao gồm:
...
- **Bảng thống kê trái**:
  ...
  - `COMBO`: Số combo hiện tại và multiplier (khi combo ≥ 2)
- Gợi ý "Press ESC to pause" (góc phải dưới)

#### Hiệu ứng:
- **Hit**: Vòng tròn xanh mở rộng + chữ "+điểm" bay lên + hiển thị combo (kéo dài 400ms)
- **Miss**: Dấu X đỏ (kéo dài 300ms)

### 3.8. Màn hình chơi Tracking (STATE_PLAYING = 5)
- **File**: `screens/tracking.py`
- Sử dụng chung state `STATE_PLAYING`, phân biệt bằng `selected_mode`

#### Target di chuyển:
- Di chuyển theo đường cong Bézier bậc 2 (quadratic)
- Tự động tạo đường đi mới khi đến đích
- Bán kính cố định: 40px

#### HUD Tracking:
- **Timer**: Thời gian còn lại (giữa trên)
- **"TRACKING MODE"**: Nhãn chế độ (dưới timer)
- **SCORE**: Góc phải trên
- **Accuracy bar**: Thanh tiến trình accuracy (dưới score)
- **Bảng thống kê trái**: Phase hiện tại, Accuracy %, Score
- **ON/OFF TARGET**: Chỉ báo real-time

#### 3 Giai đoạn tốc độ:
| Phase | Tên | Tốc độ | Thời gian |
|-------|-----|---------|----------|
| 1 | Slow | 0.2x | 1/3 game |
| 2 | Medium | 0.5x | 1/3 game |
| 3 | Fast | 1.0x | 1/3 game |
...
#### Classic Results:
  ...
  - Thống kê: Difficulty, Hits, Misses, Accuracy, Avg Reaction, Best Reaction, Best Combo
  - Nếu accuracy cao hơn kỷ lục cũ → hiện **"NEW RECORD!"**

#### Tracking Results:
  - Tiêu đề "RESULTS" + "TRACKING MODE"
  - Tổng điểm
  - Thống kê: On-Target Accuracy, Time On Target, Total Time
  - Kỷ lục lưu theo key "TRACKING"

- **2 nút**:
  - `PLAY AGAIN` → Quay lại chọn chế độ
  - `MAIN MENU` → Quay lại Menu

### 3.10. Tạm dừng (STATE_PAUSE = 7)
...
### 4.1. Công thức tính điểm (Classic Mode)

```
combo_mult = min(2.0, 1.0 + (combo - 1) × 0.1)  # Combo multiplier (max 2.0x)
Điểm mỗi hit = int((Base Points + Reflex Bonus) × combo_mult)
             = int((100 + int(max(0, TTL - reaction_time) / TTL × 50)) × combo_mult)
```
...
| **Score** | `Σ (100 + bonus) × combo_mult` cho mỗi hit | Tổng điểm tích lũy |
| **Best Combo** | Max combo đạt được | Chuỗi bắn trúng dài nhất |

### 4.5. Hệ thống kỷ lục (Records)

- Kỷ lục được lưu theo **Accuracy** (%), không phải Score
- Classic: Mỗi độ khó (Easy, Medium, Hard) có kỷ lục riêng
- Tracking: Lưu chung với key `"TRACKING"`
- So sánh: nếu `accuracy hiện tại > record[key]` → cập nhật kỷ lục
- Lưu vào file `records.json`
- Điều kiện: phải có ít nhất 1 click/thời gian chơi
...
|----------|------|--------|------|
| **Max Targets** (đồng thời) | 1 | 1 | 1 |
| **Min Lifetime** (ms) | 2000 | 1200 | 800 |
| **Max Lifetime** (ms) | 3000 | 2000 | 1500 |
| **Spawn Delay** (ms) | 200 | 150 | 100 |

> Lưu ý: Chỉ 1 target xuất hiện cùng lúc (yêu cầu MVP). Độ khó chỉ ảnh hưởng thời gian sống của target.
...
- **Tần suất spawn**: Theo spawn_delay của độ khó, chỉ spawn khi số target sống < max_targets (luôn là 1)
...
### 9.1. Tổng quan
- **File**: `screens/tracking.py`
- Target di chuyển liên tục theo đường cong Bézier bậc 2 (quadratic)
- Người chơi giữ crosshair trong vùng bán kính target (40px) để tích điểm
- Không có chọn độ khó — tốc độ tăng tự động qua 3 giai đoạn

### 7.2. TrackingTarget class
- `x, y`: Vị trí hiện tại
- `speed`: Tốc độ cơ bản (1.0), nhân với phase speed multiplier
- `radius`: Cố định 40px (`TRACKING_TARGET_RADIUS`)
- `on_target`: Boolean — crosshair có đang trong vùng target không
- Tự sinh đường cong mới khi đến đích (random control points)

### 7.3. Hệ thống 3 Phase
Thời gian game chia đều thành 3 giai đoạn:

| Phase | Tên | Tốc độ | Mô tả |
|-------|-----|--------|-------|
| 1 | Slow | 0.2x | Dễ theo dõi, làm quen |
| 2 | Medium | 0.5x | Tăng tốc vừa phải |
...
- "TRACKING MODE" label (dưới timer)
- Score (góc phải trên)
- Accuracy bar (thanh tiến trình dưới score)
- Bảng stats trái: Phase, Accuracy %, Score
- Chỉ báo ON/OFF TARGET real-time

---

## 8. Hệ thống Combo

### 8.1. Cách hoạt động (Classic Mode)
- Mỗi lần bắn trúng liên tiếp → combo tăng 1
- `combo_multiplier = min(2.0, 1.0 + (combo - 1) × 0.1)`
- Combo 1: 1.0x, Combo 2: 1.1x, ... Combo 11+: 2.0x (max)
- Miss (click trượt hoặc target hết hạn) → **reset combo về 0**

### 8.2. Hiển thị
- HUD hiển thị combo counter và multiplier khi combo ≥ 2
- Hit effect hiển thị "x{combo} COMBO!" text nổi
- Best Combo được hiển thị trong kết quả

### 8.3. Ví dụ
| Combo | Multiplier | Điểm gốc (100+37) | Điểm thực |
|-------|------------|-------------------|-----------|
| 1     | 1.0x       | 137               | 137       |
| 5     | 1.4x       | 137               | 191       |
| 11+   | 2.0x       | 137               | 274       |
...
### 9.3. Tính năng slider
- Kéo thả handle để thay đổi giá trị (knob nhỏ gọn)
...
### 9.6. Toggle switches
- Outline và Dot sử dụng **pill-style toggle switch**
- Hiển thị: "Outline:" + nút pill tròn (xanh = ON, tím đậm = OFF)
- Không có text ON/OFF — chỉ dùng màu sắc và vị trí circle indicator
- Circle trượt sang phải = ON, sang trái = OFF

---

## 10. Hệ thống âm thanh

### 10.1. Procedural Audio (Âm thanh tạo bằng thuật toán)
...
- **Combo text**: Hiển thị "x{combo} COMBO!" khi combo ≥ 2
...
### 13.2. State Machine Flow
```
MENU ─────────────→ INSTRUCTION ──→ MENU
  │                                   ↑
  ├──→ SETTINGS ──────────────────────┤
  │                                   │
  └──→ MODE_SELECT ──┬──→ DIFFICULTY ──→ COUNTDOWN ──→ PLAYING ──→ GAME OVER
                     │                      ↑           │           │
                     │                      │           ↓           │
                     │                      └─── PAUSE ──┤          │
                     │                           │       │          │
...
### 13.4. Tính năng nổi bật
- **100% Procedural**: Không sử dụng bất kỳ file asset bên ngoài nào (ảnh, âm thanh, font đặc biệt)
- **2 Game Modes**: Classic (click targets) + Tracking (follow moving target)
- **Combo System**: Consecutive hits tăng score multiplier lên đến 2.0x
- **3-Phase Tracking**: Tốc độ tăng tự động qua 3 giai đoạn (Slow → Medium → Fast)
...
- **Pill Toggle Switches**: UI hiện đại cho các tùy chọn ON/OFF
